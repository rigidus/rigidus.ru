...
void setup () {
    ...
    */
    ...
    /* Чтобы издавать звуки настроим Timer0 в режим PWM */
    /* TODO */
    /* Режим работы таймера задаётся битами WGM01 (1) и */
    /* WGM00 (0) регистра TCCR0A: */
    /* 00 - обычный режим */
    /* 01 - режим PhaseCorrectPWM */
    /* 10 - режим подсчета импульсов (сброс при совпадении) */
    /* 11 - режим FastPWM */
    /* Здесь нас интересует вариант "11" - Fast PWM */
    /* В этом режиме счётчик считает от нуля до максимума. */
    /* При установке нулевого значения счётчика - на выходе */
    /* устанавливается логическая единиц). */
    /* При совпадении с регистром сравнения устанавливается */
    /* логический ноль). В инверсном режиме - наоборот. */
    /* Биты COM0A1 (7) и COM0A0 (6) регистра TCCR0A задают, */
    /* какой сигнал появится на выводе OC0A (pin 12, PD6) */
    /* при совпадении счётчика (регистр TCNT0) с регистром */
    /* сравнения A (OCR0A). */
    /* В режиме "FastPWM": */
    /* 00 - вывод OC0A не функционирует */
    /* 01 - если бит WGM02 регистра TCCR0B установлен в 0, */
    /*      вывод OC0A не функционирует */
    /* 01 - если бит WGM02 регистра TCCR0B установлен в 1, */
    /*      изменение состояния вывода OC0A на противоположное */
    /*      при совпадении с A */
    /* 10 - установка 0 на выводе OC0A при совпадении с A, */
    /*      установка 1 на выводе OC0A при обнулении счётчика */
    /*      (неинверсный режим) */
    /* 11 - установка 1 на выводе OC0A при совпадении с A, */
    /*      установка 0 на выводе OC0A при обнулении счётчика */
    /*      (инверсный режим) */
    /* Биты COM0B1 (5) и COM0B0 (4) регистра TCCR0A задают, */
    /* какой сигнал появится на выводе OC0B (pin 11 PD5) при */
    /* совпадении счётчика (регистр TCNT0) с регистром */
    /* сравнения B (OCR0B). */
    /* В режиме "FastPWM": */
    /* 00 - вывод OC0B не функционирует */
    /* 01 - резерв */
    /* 10 - установка 0 на выводе OC0B при совпадении с B, */
    /*      установка 1 на выводе OC0B при обнулении счётчика */
    /*      (неинверсный режим) */
    /* 11 - установка 1 на выводе OC0B при совпадении с B, */
    /*      установка 0 на выводе OC0B при обнулении счётчика */
    /* (инверсный режим) */
    /* PWM_fequency = */
    /*   clock_speed / ( Prescaller_value * (1 + TOP_Value) ) */
    /* 4 кГц = 1Mhz /  8 * (1 + 100) */

    /* Настройка Timer0 для бипера */
    /* отключить глобальные прерывания */
    cli();
    ...
    TCCR0A =
        ...
        (1<<WGM01)  | /* FastPWM */
        (1<<COM0A1) |
        (0<<COM0A0) | /* non-unverse OC0A */
        (1<<COM0B1) |
        (1<<COM0B0) ; /* inverse 0C0B */
    /* /\* установка регистра совпадения *\/ */
    OCR0A = 30;
    /* включение Timer2 в FastPWM режим */
    /* остальные биты равны нулю */
    TCCR0B =
