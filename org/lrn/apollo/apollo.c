...
void setup() {
    ...
    /* TCCR1B |= (1 << CS11); */
    /* то частота тактового источника поделится на 8. */

    /* Это дает разрешение таймера 1/(1 ∗ 10^6 / 8) или 1.25e-7 с. */
    /* Теперь таймер будет переполняться каждые (65535 ∗ 1.25e-7) */
    /* или за 0.008191875 */

    /* Но есть и другой режим таймера. */

    /* Он называется сброс таймера по совпадению или CTC. */
    /* Вместо счета до переполнения, таймер сравнивает свой счетчик */
    /* с переменой которая ранее сохранена в регистре. Когда счет */
    /* совпадет с этой переменной, таймер может либо установить флаг, */
    /* либо вызвать прерывание, точно так же как и в случае переполнения. */

    /* Чтобы использовать режим CTC надо понять, сколько циклов вам нужно, */
    /* чтобы получить интервал в одну секунду. Предположим, что коэффициент */
    /* деления по-прежнему равен 8. */
