...
Наш предыдущий лисп уже имеет ~eval~, ~repl~ и может вычислять списковые формы. Однако
в нем пока еще невозможно сделать функцию. Чтобы добавить эту функциональность нам как
минимум нужно уметь определять лямбды.

Определение лямбд, которые умеют принимать параметры, влечет за собой необходимость
иметь средство сопоставить формальные параметры - фактическим. Одним из таких средств
являются "окружения" - структура данных, которая представляет собой список точечных
пар, где первая половина пары содержит символ, а вторая - его значение. Такую структуру
мы можем поместить в глобальную переменную и получить таким образом "глобальное
окружение", которое могут изменять формы ~defun~ и ~setf~. Для простоты мы будем
помещать и переменные и функции в одно и то же глобальное окружение, таким образом наш
лисп будет являться ~lisp1~.

Здесь у нас появляется разделение на функции и специальные операторы. В отличии от
функций, специальные операторы могут не вычислять свои аргументы.

Чтобы применять функции к их вычисленным аргументам мы добавим ~myapply~.В ~myapply~ мы
будем помещать встроенные функциии, а в ~myeval~ останутся специальные операторы.

Кроме того мы отделим классы ошибок интерпретатора от всего остального кода для
удобства обработки
...
аргументы. Поэтому им не нужно вызывать ~myeval~ для аргументов.
...
совпало с формой. Вместо этого мы предоплагаем, что это вызов функции, а значит
...
вызов ~evlic~. См раздел [[*LIST][LIST]] для подробностей.
