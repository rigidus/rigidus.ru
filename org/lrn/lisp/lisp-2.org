...
сначала в локальном, а потом (если в локальном символ не найдет) и в глобальном
окружении.
...
#+NAME: errors_2
#+BEGIN_SRC lisp
  (define-condition invalid-number-arguments (error)
...
#+BEGIN_SRC lisp
  ((equal fn 'null)            (if (null (cdr args))
                                   (null (car args))
                                   (error 'invalid-number-arguments :fn fn)))
#+END_SRC
