...
~lookup~. Это функция, которая ищет (и вовзращает) значение переданного ей символа
сначала в локальном, а потом (если в локальном не нашла) - в глобальном окружении.
...
единственный и возможно не самый лучший способ получить ~defun~ и ~setq~, за
продробностями стоит идти [TODO:gmm] куда?
...
рекурсивное вычисление форм в конец ~cond~ в ~myeval~. Если мы не сделаем это - что
произойдет? [TODO:gmm]
