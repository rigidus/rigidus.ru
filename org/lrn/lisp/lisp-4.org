...
CPS-преобразовании. Оно дает нам несколько бесплатных бонусов, напримимер:
...
что делает эвлист - вычисляет аргументы функции

потом мы взяли последний эвлис который я написал

и сделали соответствующую часть евала:
...
и здесь есть продолжение, которое не является шагом вычисления в интерпретаторе

как пример интерпретатор https://gist.github.com/fogus/3698078 в нем есть 3
продолжения, одно из которых является лишним и чтобы его не делать перед
cps-преобразованием мы выполнили хвосторекурсивное преобразование с аккумулятором, что
дало нам возможность получить более хороший результат - с двумя продолжениями вместо
трех.

Если взять евлис и преобразовать его в цпс то получится вот так:
...
Для этого мы переместим функционал аплая - применение функции к аргументам прямо в
евлис, передав ему функцию, которую будем аплаить. тогда выйдет вот так:
