...
т.е. пока ~n~ не станет равным нулю.
...
Преобразум вызов в хвосторекурсивный, используя аккумуляторы

#+BEGIN_SRC lisp
  (defun ftc (n &optional (acc1 1) (acc2 1))
    (cond ((or (equal 1 n) (equal 2 n))  acc1)
          (t                             (ftc (- n 1)  (+ acc1 acc2)  acc1))))
#+END_SRC
...
Честно говоря я сам не понял как я это сделал и почему на конце там ~(lambda (x)
x)~. [TODO:gmm] Ну, то есть это такой способ возвратить число из вызова ~(ftc-cps (-
n 2) ...)~, чтобы потом сложить его с ~x~. Можно в целом еще более приблизиться, если
использьзовать параметр по умолчанию:

#+BEGIN_SRC lisp
  (defun ftc-cps-d (n &optional (cont (lambda (x) x)))
    (cond ((equal 1 n)  (funcall cont 1))
          ((equal 2 n)  (funcall cont 1))
          (t            (ftc-cps-d (- n 1)
                                   (lambda (x)
...
~ftc~ "следующей исполняющейся части" и формированием из нее продожения.

#+BEGIN_SRC lisp
  (defun ftc-cps-2 (n cont)
    (cond ((equal n 1)  (funcall cont 1))
...
#+BEGIN_SRC lisp
  (defun evlis (lst env errcont cont)
    (cond ((null lst)  nil)
          (t           (cons (myeval (car lst) env errcont cont)
                             (evlis (cdr lst) env errcont cont)))))
#+END_SRC

Теперь переименуем параметр ~lst~ в ~unevaled~, добавим параметр ~evaled~ и преобразуем
~evlis~ так-же как мы делали с ~mul2~, чтобы получить хвостовую рекурсию:
...
Мы можем переформулировать вызов ~myeval~ так, чтобы выделить его как отдельную
лямбда-функцию, которая обрабатывает элемент функции перед передачей его в ~evaled~:

#+BEGIN_SRC lisp
  (defun evlis (unevaled evaled env errcont cont)
    (cond ((null unevaled)  (reverse evaled))
          (t                (evlis (cdr unevaled)
                                   (cons (funcall (lambda (x)
...
Выполним (так же как и с ~mul2~) выделение лямбда-функции в отдельную функцию ~fn~,
которая обрабатывает элемент списка при передаче его из ~unevaled~ в ~evaled~.
...
Так, я только что обратил внимание, что результирующий ~evlis~ не хвостовой. Попробую
вернуться назад к:

#+BEGIN_SRC lisp
  (defun evlis (unevaled env errcont cont)
    (cond ((null unevaled)  nil)
          (t           (cons (myeval (car unevaled) env errcont cont)
                             (evlis (cdr unevaled) env errcont cont)))))
#+END_SRC

и сразу провести CPS-преобразование от ~myeval~:

#+BEGIN_SRC lisp
  (defun evlis (unevaled env errcont cont)
    (cond ((null unevaled)  nil)
          (t           (cons (myeval (car unevaled) env errcont
                                     (lambda (x)
                                       (evlis (cdr unevaled) env errcont
                                              (lambda (y)
                                                (cons x y)))))))))
#+END_SRC

Все равно какая-то херня получается...

Непонятно как выбрать путь преобразований и почему он должен быть выбран именно в эту сторону
