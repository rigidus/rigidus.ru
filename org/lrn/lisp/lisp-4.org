...
[TODO:gmm] Преобразовать в CPS с аккумулятором
...
[TODO:gmm] Так как у нас теперь есть EVLIS, мы наконец можем отказаться от вызова
~myeval~ в EVADD и EVMUL

#+NAME: myapply_ariph_4
#+BEGIN_SRC lisp
  ((equal fn '+)               (funcall cont (apply #'+ args)))
  ((equal fn '*)               (funcall cont (apply #'* args)))
#+END_SRC
