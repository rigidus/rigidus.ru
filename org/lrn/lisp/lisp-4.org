...
Добавим LIST, который возвращает свои аргументы
...
просто вызовет другое error-продолжение. Поэтому класс ошибки нам больше не требуется.
...
используем ~evlet~ в ~apply~ чтобы вычислить ~let~
