...
#+BEGIN_SRC lisp
  (defun myapply (fn args errcont cont)
    (cond
      ...
      <<myapply_null_4>>
      <<myapply_print_4>>
      <<myapply_or_4>> ;; or не должен быть здесь потому что он специальный оператор
      <<myapply_and_4>>
      <<myapply_list_4>>
      <<myapply_closure_4>>
      ))
...
** TODO Встроенные функции арифметики

[TODO:gmm] Тут надо доделать

Вспомогательные функции ~evadd~ и ~evmul~ теперь тоже принимают продолжения:

#+NAME: evaddmul_3
#+BEGIN_SRC lisp
  (defun evadd (lst env cont)
    (cond ((null lst)        (+))
          ((null (cdr lst))  (+ (myeval (car lst) env cont)))
          (t                 (+ (myeval (car lst) env cont)
                                (evadd (cdr lst) env cont)))))
  (defun evmul (lst env cont)
    (cond ((null lst)        (*))
          ((null (cdr lst))  (* (myeval (car lst) env cont)))
          (t                 (* (myeval (car lst) env cont)
                                (evmul (cdr lst) env cont)))))
#+END_SRC
...
LIST - это функция, т.к. она оценивает свои аргументы. Перенесем LIST из ~myeval~ в
~myapply~ [TODO:gmm] и научим его принимать продолжения:
...
#+BEGIN_SRC lisp
  ...
  ;; (assert (equal '(3 6 42) (evlis '+     '(1 (+ 1 2) (* 2 3) 42)  nil nil #'err #'ok)))
...
рекурсивном вызове внутри лямбды. Мы также переименовыываем параметр ~lst~ в ~exp~.
...
LIST - это функция, поэтому она должна обрабатываться в MYAPPLY. Туда мы ее и
перенесли.
