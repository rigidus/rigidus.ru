...
|-----------------------------------+--------+--------+-------------|
| (= NEW $LOW)                      | Up     | Up     |             |
| (= NEW $HIGH)                     | Up     | Middle | (clear CNT) |
| (= NEW $LOW)                      | Middle | Up     |             |
| (and (= NEW $HIGH) (< CNT $MAX))  | Middle | Middle | (inc CNT)   |
...
(("Up"
  ...
  (:to "Middle" :predicate "(= NEW $HIGH)" :action "(clear CNT)"))
 ("Middle"
  (:to "Up" :predicate "(= NEW $LOW)" :action "")
  (:to "Middle" :predicate "(and (= NEW $HIGH) (< CNT $MAX))" :action "(inc CNT)")
...
переменной =NEW= с константой. При этом во всех случаях константа идет
следом за переменной, а переменная может быть либо =$LOW= либо
=$HIGH=. Это упрощает написание правила, т.к. мы можем использовать
предустановленные значения =0= и =1= в регистрах =r0= и =r1=.
...
#+BEGIN_SRC elisp :var a=recur
  ...
  (add-recur-rule (lambda (lst)
                    (and  (eql '= (car lst))
                          ...
                          (equal 'NEW (cadr lst))
                          (or (equal '$LOW (caddr lst))
                              (equal '$HIGH (caddr lst)))))
                  (lambda (lst)
                    (let ((const (caddr lst))
  ...
  (add-recur-rule (lambda (lst)
                  ...
                  (lambda (lst)
                    (let ((base       (gensym "reg=case_base_"))
                      ...
                      (push `(ldi ,offset 12) ssa)         ; 4 CONST=12
                      (push `(add ,low ,offset) ssa)       ; 5
                      (push `(adc ,high ,zero) ssa)        ; 6
                      (push `(rfree ,offset) ssa)
                      ;; + base
                      (push `(ralloc ,base) ssa)
                      (push `(ld  ,base ,(cadr lst)) ssa)  ; 7
                      (push `(add ,low ,base) ssa)         ; 8
                      (push `(adc ,high ,zero) ssa)        ; 9
                      (push `(rfree ,base) ssa)
                      ;; zero -off
                      (push `(rfree ,zero) ssa)
                      ;; indirect jump
                      (push `(push ,low) ssa)             ; 10
                      (push `(rfree ,low) ssa)
                      (push `(push ,high) ssa)            ; 11
                      (push `(rfree ,high) ssa)
                      (push `(ret) ssa)                   ; 12
                      ;; jump table
                      (mapcar #'(lambda (x)
...
#+NAME: all
#+BEGIN_SRC elisp :var z=pseudocode a=recur b=rnopprogn c=rclear d=req2 e=rsetstate f=rand g=rwhen h=rless i=rinc j=rge k=rcase
  (format
   "%s"
   (pp
...
последней выполненной операции, =tmp0= и =freq=. Все они определены у нас
в дефайнах
...
Конечный автомат обработки временных интервалов должен срабатывать, когда
состояние кнопки изменяется. Для этого ему нужно хранить предыдущее
состояние кнопки, которое тоже должно быть инициализировано на старте
