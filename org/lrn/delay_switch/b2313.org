...
|---------------------------------------+--------+--------+--------------|
| (and (= PREV $LOW) (= NEW $HIGH))     | Up     | Middle | (setf CNT 0) |
| (and (= PREV NEW $HIGH) (< CNT $MAX)) | Middle | Middle | (incf CNT)   |
| (and (= PREV NEW) (>= CNT $MAX))      | Middle | Down   | (setf CNT 0) |
| (= PREV NEW $HIGH)                    | Down   | Down   |              |
| (and (= PREV $HIGH) (= NEW $LOW))     | Down   | Up     | (setf CNT 0) |
| (and (= PREV NEW $LOW))               | Up     | Up     |              |
...
#+results: grouping
: (("Up"
:   (:to "Middle" :predicate "(and (= PREV $LOW) (= NEW $HIGH))" :action "(setf CNT 0)")
:   (:to "Up" :predicate "(and (= PREV NEW $LOW))" :action ""))
:  ("Middle"
:   (:to "Middle" :predicate "(and (= PREV NEW $HIGH) (< CNT $MAX))" :action "(incf CNT)")
:   (:to "Down" :predicate "(and (= PREV NEW) (>= CNT $MAX))" :action "(setf CNT 0)"))
:  ("Down"
:   (:to "Down" :predicate "(= PREV NEW $HIGH)" :action "")
:   (:to "Up" :predicate "(and (= PREV $HIGH) (= NEW $LOW))" :action "(setf CNT 0)")))
...
#+BEGIN_SRC asm :tangle b2313.S :noweb yes :padline no
  ...
  _mainloop:
      lds     tmp0, STATE
      ldi     tmp1, 2             ; Проверяем на состояние 2
      cp      tmp0, tmp1
      brne    _not_one            ;--+
      rcall   _red_on             ;--|-+
      rjmp    _end_no_one         ;  | |
