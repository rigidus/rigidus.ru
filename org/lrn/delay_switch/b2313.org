...
#+BEGIN_SRC asm :var depends=asm_expander :noweb yes :padline no
  ...
  <<chain_of_fsm(id="SP", inport="A", inbit=1, outport="D", outbit=5)>>
  <<chain_of_fsm(id="PP", inport="A", inbit=0, outport="D", outbit=4)>>
  <<chain_of_fsm(id="PM", inport="D", inbit=2, outport="D", outbit=3)>>
#+END_SRC
...
Аналогично, по тем же причинам надо очистить переменную =BEEP_@=.
...
#+BEGIN_SRC asm :var id="@" port="@" bit="@"
  ...
  _switch_off_&id:
      OFF PORT&port &bit
  _switch_end_&id:
  ;;     ;; Включаем/выключаем таймер-0 в зависимости от BEEP
  ;;     lds     tmp0, BEEP_&id
  ;;     cp      tmp0, r0
  ;;     breq    _beep_off_&id
  ;;     out TCCR0B, r1
...
| (2>= Z $SIGNAL_TIME) | Run  | Run  | (setout)   |
