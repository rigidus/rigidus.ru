...
** START Обработка временных интервалов
...
Ддя этого предыдущий автомат формирует выходное воздействие, которое
...
должны перевести его в состояние выполнения программы =Run= и включить
нагрузку. А если он находится в состоянии выполнения программы - то
перевести в состояние ожидания =Wait= и выключить нагрузку.
...
| predicate         | from | to   | action                         |
|-------------------+------+------+--------------------------------|
| (= Y 1)           | Wait | Run  | (progn (clear Z) (on PORTD 6)) |
| (= Y 1)           | Run  | Wait | (off PORTD 6)                  |
| (>= Z $DELAY_MAX) | Run  | Wait | (off PORTD 6)                  |
...
#+results: prg_group
: (("Wait"
:   (:to "Run" :predicate "(= Y 0)" :action "(progn (clear Z) (on PORTD 6))"))
:  ("Run"
:   (:to "Wait" :predicate "(= Y 0)" :action "(on PORTD 6)")
:   (:to "Wait" :predicate "(>= Z $DELAY_MAX)" :action "(off PORTD 6)")))
...
#+NAME: all_two
#+BEGIN_SRC elisp :var z=prg_code a=recur b=rnopprogn c=rclear d=req2 e=rsetstate f=rand g=rwhen h=rless i=rinc j=rge k=rcase l=ron m=roff
...
#+BEGIN_SRC asm :noweb yes
  ...
  <<gen_get_btn("IP", "B", 0)>>
      ...
      ;; Передаем адрес переменной состояния нового автомата
      ldi     xl, lo8(MODE_IP)
      ldi     xh, hi8(MODE_IP)
      ...
      ;; изменение STATE. Поэтому нам нужно PREV
      ;; Тогда мы можем переключать другой автомат когда
      ;; STATE меняется.
      ;; Теперь если PREV и STATE не совпадают -
