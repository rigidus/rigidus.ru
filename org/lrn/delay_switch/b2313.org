...
Instruction Pointer командой CALL, потом вынем его в регистр. Если теперь
взять адрес начала таблицы и вычесть из него полученный на предыдущем
шаге адрес Instruction Pointer - то мы получим смещение до таблицы от
точки, где мы получили IP.
