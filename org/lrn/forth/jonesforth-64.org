...
#+BEGIN_SRC c :tangle src/src64/runvfm64.c :noweb tangle :exports code :padline no :comments none
  ...
  void runvfm (vfm base code params env run hash)
  {
      int pid;
      int in, out, cnt;
      int inPipe[2], outPipe[2];
      if (pipe(inPipe)  == -1) { perror("Pipe Failed"); exit(-1); }
      if (pipe(outPipe) == -1) { perror("Pipe Failed"); exit (-1); }
      switch(pid = fork()) {
      case -1:
          perror("fork"); exit(-1);
      case 0:
  ...
  {
      runvfm("./forth64",
             read_file_into_string("./lib.f"),
             ": ALFA .\" ᚜do-beta-gamma᚛\" CR ;"
             { "forth64", "asd", "qwe" NULL },
             { "USER=test", "HOME=/home/test", NULL },
             "call",
             "hash"
             );
      return 0;
  ...
  /*
      (run-vfm
       (format nil "~Avfm" *base-dir*)
       (read-file-into-string (format nil "~Alib.f" *base-dir*))
       code  ;; ": ALFA .\" ᚜do-beta-gamma᚛\" CR ;"
       '("asd" "qwe") (list (format nil "SENDER=~A" sender)
                                 (format nil "AMOUNT=~A" amount)
                                 (format nil "INPUT=~A"  input))
            call hash)
  ,*/
      /* char result[SIZE]; */
