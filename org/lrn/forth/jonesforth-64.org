...
#+BEGIN_SRC c :tangle src/inc64/asm.h :noweb yes :exports code :padline no :comments none
...
#+BEGIN_SRC c :tangle src/src64/main64.c :noweb yes :exports code :padline no :comments none
...
#+BEGIN_SRC asm
      defcode "KEY",3,,KEY
      call _KEY
      push    %rax            #       # push return symbol to data stack
      NEXT                    #
      .align 8
  currkey:
      # Stores the offset to the current position in the input buffer
      # (the next character will be read from it)
      .quad input_buffer
  bufftop:
      # Stores the top of the input buffer (last valid data + 1)
      .quad input_buffer
  _KEY:
      pop     %rcx            #       # %rcx <- retaddr
      push    %rcx            #       # restore retaddr
      add     $(5+(wordsize-1)), %rcx # BASE + 1(push-%rax) + 4(NEXT) + 7(wordsize-1)
      and     $~(wordsize-1), %rcx    # NOW %rcx => currkey
  _KEY_REPEAT:                # <--+
      mov     (%rcx), %rbx    #    |  # (currkey) -> %rbx
      cmp     8(%rcx), %rbx   #    |  # (bufftop >= currkey)? - are there symbols in the buffer?
      jge     1f              #-+  |  # ?-NO, go forward
      xor     %rax, %rax      # |  |  # ?-YES,
      mov     (%rbx), %al     # |  |  #    (1) (currkey) -> %rax,
      inc     %rbx            # |  |  #    (2) increment copy of currkey %rbx
      mov     %rbx, (%rcx)    # |  |  #    (3) save copy of currkey %rbx to var currkey,
      ret                     # |  |  #    (4) return (%rax contains symbol)
      # ---------------- RET    |  |
  1:  #                     <---+  |  # Buffer is empty, need read from stdin
      push    %rsi            #    |  # save %rsi & %rdi before syscall
      push    %rdi            #    |  #
      mov     $stdin, %rdi    #    |  #  param1: stdin in %rdi
      mov     $input_buffer, %rsi #|  #  param2: addr in %rsi
      mov     %rsi, (%rcx)    #    |  #  save addr to currkey
      mov     $INPUT_BUFFER_SIZE, %rdx # param3: max length size in %rdx
      mov     $sys_read, %rax #    |  #  SYSCALL read in %rax
      syscall                 #    |  #  SYSCALL
      # Проверяем возвращенное     |  # должно быть количество символов + '\n'
      test    %rax, %rax      #    |  # (%rax <= 0)?
      jbe     2f              #-+  |  # ?-YES, this is an error, go forward
      add     %rax, %rsi      # |  |  # ?-NO, (1) add %rsi count read bytes
      mov     %rsi, 8(%rcx)   # |  |  #       (2) save %rsi to bufftop
      pop     %rdi            # |  |  # restore %rdi & %rsi
      pop     %rsi            # |  |
      jmp     _KEY_REPEAT     # |  |
      # ------------------------|--+
  2:  #                     <---+     # Error or Ctrl+C - EXIT
      pop     %rdi            #       # restore %rdi & %rsi
      pop     %rsi            #
      mov     $sys_exit, %rax         # param1: SYSCALL #1 (exit)
      xor     %rdi, %rdi              # param2: retcode = 0
      syscall                         # SYSCALL
      # --------------- EXIT
      .data
      .align 8
...
#+NAME: ext_control
#+BEGIN_SRC c :tangle src/src64/runvfm64.c :noweb tangle :exports code :padline no :comments none
  #include <unistd.h>
  #include <stdio.h>
  #include <stdlib.h>
  #include <fcntl.h>
...
#+NAME: ext_control
#+BEGIN_SRC c :tangle src/src64/node64.c :noweb tangle :exports code :padline no :comments none
  #include <stdio.h>
  #include <stdlib.h>
...
#+NAME: assembly
#+BEGIN_SRC asm :tangle src/src64/jonesforth64.s :noweb yes :exports code :padline no :comments none
  <<flags>>
