...
#+BEGIN_SRC lisp
  ...
  ;; test for evprogn
  (assert (eqaul 2 (evprogn '(1 2) nil #'err #'ok)))


  ;; evletstar
  (defun evletstar (varpairs exp env errcont cont)
    (cond ((null varpairs) (myeval exp env errcont cont))
          (t (myeval (car (cdr (car varpairs))) env errcont
                     (lambda (x)
  ...
  ;; test for evletstar
  (assert (equal 2 (evletstar '((a 1) (b a)) '(+ a b) nil  #'err #'ok)))
  ...
  (defun evlet (vars exps evald-exps exp env errcont cont)
    (cond ((null exps)                      (myeval exp (pairlis vars (reverse evald-exps) env) errcont cont))
          (t (myeval (car exps) env errcont
                     (lambda (x)
                       (evlet vars (cdr exps) (cons x evald-exps) exp env errcont cont))))))

  ;; test for evlet
  (assert (equal 3 (evlet '(a b) '(1 2) nil '(+ a b) nil #'err #'ok)))


  ;; myeval
  (defun myeval (exp env errcont cont)
    (cond ((numberp exp)                    (funcall cont exp))
          ((member exp '(+ * car cdr cons
                         null print))       (funcall cont exp))
          ...
          ((equal (car exp) 'let*)          (evletstar (cadr exp) (caddr exp) env errcont cont))
