...
#+BEGIN_SRC lisp
  ...
  ;;         (set-red image)
  ;;       (save-png dw dh "cell2.png" (vectorize-image new-image))
  ;;       red-point)))

  (defun modify-red-neighbors (px py image)
    (let ((result)
       ...
       :do (cond ((and (equal (aref image dy dx 0) 0) ;; if black
                  ...
                  (progn ; new red point
                    (format t "~%>:~A|~A = ~A.~A.~A"
                            ...
                            (aref image dy dx 2))
                    ;; push new red point
                    (push (list dx dy) result)
                    ;; set new red point in image (instead black)
                    (setf (aref image dy dx 0) 255)
                    ;; remove basic red point from image
                    (setf (aref image py px 0) 0)
                    (setf (aref image py px 1) 255)
                    (setf (aref image py px 2) 0)
                    ))
                 (t nil)))
      (values image result)))
