...
#+BEGIN_SRC lisp :tangle sn.lisp :noweb tangle :padline no
  ...
  (let* ((from "text.png")
    ...
    (loop
       :for box :in boxes
       :for idx :from 0 :to (length boxes) :by 2
       :do (let ((threshold-y 10)
                 (threshold-x 20))
             (setf image-data
    ...
    (destructuring-bind ((blu-y . blu-x) (brd-y . brd-x) (bcc-y . bcc-x))
      ...
      (block neighbour
        (loop :for ((tlu-y . tlu-x) (trd-y . trd-x) (tcc-y . tcc-x)) :in tail :do
             ...
             (when (and (> tcc-x bcc-x)
                        (< tcc-x (+ bcc-x threshold-x))
                        (< tcc-y (abs (+ bcc-y threshold-y)))
                        )
