...
#+BEGIN_SRC lisp
  ...
  (defun binarization (image &optional threshold)
    (let* ((dims (array-dimensions image))
           ...
           (result (make-array dims :element-type '(unsigned-byte 8))))
      (macrolet ((cycle (&body body)
                   `(do ((y 0 (incf y)))
                        ((= y (array-dimension result 0)))
                      (do ((x 0 (incf x)))
        ...
        (cond ((equal 3 (length dims))
               (cycle (do ((z 0 (incf z)))
                          ((= z (array-dimension result 2)))
                        (let ((avg (floor (+ (aref image y x 1)
                                             (aref image y x 0)
                                             (aref image y x 2))
                                          3)))
                          (when threshold
  ...
  (let* ((from "~/Pictures/snap5.png")
         (to   "~/Pictures/snap6.png")
  ...
  (let* ((from "~/Pictures/snap8.png")
         (to   "~/Pictures/snap9.png")
