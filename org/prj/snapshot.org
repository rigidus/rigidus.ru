...
#+BEGIN_SRC lisp :tangle sn.lisp :noweb tangle :padline no
  ...
  (let* ((from "text.png")
    ...
    ;; draw center
    (setf boxes
          (loop :for (left-up right-down) :in boxes :collect
               (let* ((diff (cons (- (car right-down) (car left-up))
                 ...
                 (list left-up right-down center)
                 )))
    ;; reverse boxes for reading order
    (setf boxes (reverse boxes))
    ;; for all boxes to find nearest neighbour
    (loop
       :for box :in boxes
       :for idx :from 3 :to 5
       :do (setf image-data
                 (nearest-neighbour image-data
                                    (nth idx boxes)
                                    (nthcdr (+ 1 idx) boxes) 15)))
    ;; draw picture
    (destructuring-bind (height width)
        (array-dimensions image-data)
      (save-png width height to image-data :grayscale)))

  (defun nearest-neighbour (image-data box tail threshold)
    (destructuring-bind ((blu-x . blu-y) (brd-x . brd-y) (bcc-x . bcc-y))
        box
      (print (list (cons blu-x blu-y)
                   (cons brd-x brd-y)
                   'a))
      (block neighbour
        (loop :for ((tlu-x . tlu-y) (trd-x . trd-y) (tcc-x . tcc-y)) :in tail :do
             (when t
               ;; (and (> tcc-x bcc-x)
               ;;      (> tcc-x (+ bcc-x threshold))
               ;;      (< (abs (- bcc-y tcc-y)) threshold))
               (return-from neighbour
                 (progn
                   (print (list (cons tlu-x tlu-y)
                                (cons trd-x trd-y)
                                'b))
                   (setf image-data
                         (draw-box image-data
