...
#+BEGIN_SRC lisp :tangle sn.lisp :noweb tangle :padline no
  ...
  (let* ((from "text.png")
    ...
    (loop :for ((blu-y . blu-x) (brd-y . brd-x) (bcc-y . bcc-x)) :in boxes :do
         (let* ((box-height  (- brd-y blu-y))
                (box-width   (- brd-x blu-x))
                (sprite-data (let ((result (make-array
                                            (list (+ 1 box-width) (+ 1 box-height))
                                            :element-type '(unsigned-byte)
                                            :initial-element 100)))
                               (print (list box-height box-width))
                               (loop :for dx :from blu-x :to brd-x
                                  :for nx :from 0 :do
                                    (loop :for dy :from blu-y :to brd-y
                                       :for ny :from 0 :do
                                         (progn
                                           (format
                                            t "~%nx:~3,'0D : ny:~3,'0D | dx:~3,'0D : dy:~3,'0D |  ~3,'0D"
                                            nx ny dx dy (aref image-data nx ny))
                                           (setf (aref result nx ny)
                                                 ;; (aref image-data dy dx)
                                                 1
                                                 )
                                         ))
                                    (print ".")
         ...
         ;;        (image (skippy:make-image
         ;;                :height height :width width :image-data sprite-data
         ;;                :delay-time 10 :disposal-method :restore-previous
         ;;                :transparency-index 255
                ;;                :top-position (cdr left-up) :left-position (car left-up)))
                )
           ;; (add-image gif-image data-stream)
           )
         )
    ...
    ;;      (setf image-data (draw-box image-data left-up right-down)))
    ;;
    ;; ;; reverse boxes for reading order
    ;; (setf boxes (reverse boxes))
    ;; ;; for all boxes
    ;; (loop
    ;;    :for box :in boxes
    ;;    :with prev-box
